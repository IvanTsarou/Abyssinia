...
function createLocationCard(location) {
    ...
    var imgHtml = photo
        ? '<img src="' + photo + '" alt="' + location.name + '" class="w-full h-full object-cover" loading="lazy" onerror="this.parentElement.innerHTML=\'<div class="flex items-center justify-center bg-gray-100 text-gray-400 h-full"><i class="fas fa-image text-3xl"></i></div>\'">'
    ...
    return '<div class="location-card bg-white rounded-xl shadow-lg overflow-hidden" data-location-id="' + location.id + '" data-region="' + (location.region || '') + '">' +
        '<div class="relative h-48 sm:h-56 overflow-hidden">' + imgHtml + '</div>' +
        '<div class="p-4">' +
        '<h3 class="text-lg font-bold text-gray-800 mb-1">' + location.name + '</h3>' +
        '<p class="text-gray-500 text-sm mb-2">' + (location.region || '') + '</p>' +
        '<p class="text-gray-600 text-sm leading-relaxed line-clamp-3 mb-3">' + (location.description || '').substring(0, 150) + '…</p>' +
        ...
        '<a href="location.html?id=' + location.id + '" class="flex-1 bg-[#078930] hover:bg-[#056b24] text-white font-medium py-2 px-3 rounded-lg text-center text-sm" onclick="event.stopPropagation()"><i class="fas fa-info-circle mr-1"></i>Подробнее</a>' +
        '<button type="button" onclick="event.stopPropagation();showOnMap(' + location.id + ');" class="flex-1 bg-[#DA121A] hover:bg-[#b80e15] text-white font-medium py-2 px-3 rounded-lg text-sm"><i class="fas fa-map-marker-alt mr-1"></i>На карте</button>' +
        '</div></div></div>';
}
...
function renderLocationCards(filteredLocations) {
    ...
    container.innerHTML = list.map(createLocationCard).join('');
